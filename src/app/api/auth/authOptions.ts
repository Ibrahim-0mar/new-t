...
export const authOptions: AuthOptions = {
  ...
  secret: process.env.NEXTAUTH_SECRET || 'uIoSX+apnsd2xJFnVZwPTG8oeghM3QGkJTiopky3jG0',
  ...
  callbacks: {
    ...
    signIn: async ({ account, profile }) => {
      if (account?.provider === 'google') {
        ...
        try {
          const { data } = await travolicAxios.post('/auth/login-google', {
            tokenId: account.id_token,
          });
          account.apiResponse = { isNewUser: data.isNewUser, ...data.payload };

          return true;
        } catch (err) {
          throw new Error('Error in google signIn callback', err);
        }
      } else if (account?.provider === 'facebook') {
        try {
          const { data } = await travolicAxios.post('/auth/login-facebook', {
          ...
          throw new Error('Error in facebook signIn callback', err);
        }
      }
