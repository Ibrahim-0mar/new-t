...
export async function generateMetadata({
  ...
  const props: DynamicPageSectionProps = {
    origin: { code: originCode, type: 'airport', name: origin },
    destination: {
      code: destinationCode,
      type: 'airport',
      name: destination,
  ...
  return {
    ...
    alternates: {
      canonical: `/${locale}/cheapest-flights/routes/airports/${formatString(data[0])}/${formatString(data[1])}`,
      languages: languagesMap.reduce((acc: any, language: any) => {
        acc[language.code] =
          `/${language.code}/cheapest-flights/routes/airports/${formatString(data[0])}/${formatString(data[1])}`;
        return acc;
      }, {}),
    },
  };
...
const AirportToAirport = async ({
  ...
  const props: DynamicPageSectionProps = {
    origin: { code: originCode, type: 'airport', name: originName },
    destination: {
      code: destinationCode,
      type: 'airport',
