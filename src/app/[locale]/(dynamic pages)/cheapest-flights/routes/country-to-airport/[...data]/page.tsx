...
export async function generateMetadata({
  ...
  const title = cheapestPrice
    ? t(
      ...
      { origin: countryName, destination: airportName, cheapestPrice },
      {
        number: {
    ...
    : t('nxqQoiYaiemvILGQjldog', { origin: countryName, destination: airportName });

  return {
    title,
    description: t('RE-YUAfsmUv2rzCVsdOgp', { origin: countryName, destination: airportName }),
    alternates: {
      canonical: `/${locale}/cheapest-flights/routes/country-to-airport/${formatString(data[0])}/${formatString(data[1])}/${formatString(data[2])}`,
      languages: languagesMap.reduce((acc: any, language: any) => {
        acc[language.code] =
          `/${language.code}/cheapest-flights/routes/country-to-airport/${formatString(data[0])}/${formatString(data[1])}/${formatString(data[2])}`;
        return acc;
      }, {}),
    },
  };
  ...
  return (
    <>
      <SearchboxSection
        ...
        data={{ origins: [countryName], destinations: [airportCode] }}
        isDynamicPage={true}
      ...
      <Container>
        ...
        <h1 className="header mt-6">
          {t('R8wEEgs-2oVSDMB2D878Y', { origin: countryName, destination: airportName })}
        </h1>
        <p className="subHeader">
          {t('M0txayElVgfeC0oklDMTJ', { origin: countryName, destination: airportName })}
        </p>
...
export default CountryToAirport;
