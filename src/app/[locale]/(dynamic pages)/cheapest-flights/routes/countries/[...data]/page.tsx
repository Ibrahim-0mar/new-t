...
export async function generateMetadata({
  ...
  const props: DynamicPageSectionProps = {
    origin: { code: originCode, type: 'country', name: origin },
    destination: {
      code: destinationCode,
      type: 'country',
      name: destination,
  ...
  return {
    ...
    alternates: {
      canonical: `/${locale}/cheapest-flights/routes/countries/${formatString(data[0])}/${formatString(data[1])}/${formatString(data[2])}`,
      languages: languagesMap.reduce((acc: any, language: any) => {
        acc[language.code] =
          `/${language.code}/cheapest-flights/routes/countries/${formatString(data[0])}/${formatString(data[1])}/${formatString(data[2])}`;
        return acc;
      }, {}),
    },
  };
...
const CountryToCountry = async ({
  ...
  const props: DynamicPageSectionProps = {
    origin: { code: originCode, type: 'country', name: originName },
    destination: {
      code: destinationCode,
      type: 'country',
  ...
  return (
    <>
      <SearchboxSection
        ...
        data={{ origins: [originName], destinations: [destinationName] }}
        isDynamicPage={true}
