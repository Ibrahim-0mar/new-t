...
// import AirportTransfersSearchResultsHome from "@/views/common/results/AirportTransfers";
...
//   const dates = params.legs.map(leg => {
//     const [date, time] = leg.split('_');
//     const decodedTime = decodeURIComponent(time);
//     return new Date(`${date}T${decodedTime}`);
//   })

//   // Create an array that transforms the legs into 'from Leg to Leg' string shape
//   const origin = searchParams.origin_name as string || 'Cairo International Airport (CAI)';
...
//   // Loop over the dates to format and group it in a single string
//   const routesDates = dates.reduce((acc, curr, i) => {
...
//   const title = t('FZzviizqHz23Fpxcpo_Wq', { origin, dates: routesDates });
//   const description = t('9WTsBx0QFxuTwqG0_ViOT', { origin, dates: routesDates });
...
//     description,
//     alternates: {
//       canonical: `/${params.locale}/airport-transfers/search/${params.legs.join('/')}?${searchQuery.toString()}`,
//       languages: languagesMap
//         .filter((lang) => lang.code !== params.locale)
//         .reduce((acc: any, language: any) => {
//           acc[language.code] = `/${language.code}/airport-transfers/search/${params.legs.join('/')}?${searchQuery.toString()}`;
//           return acc;
//         }, {}),
//     },
//     openGraph: {
//       title,
//       description,
//       url: `https://travolic.com/${params.locale}/airport-transfers/search/${params.legs.join('/')}?${searchQuery.toString()}`,
//       siteName: 'Travolic',
//       images: [
//         {
//           url: '/ogImage.png',
...
// export default function AirportTransfersResults() {
//   return <AirportTransfersSearchResultsHome />
// }
...
export default function AirportTransfersSearchResultsHome() {
  ...
  const _getRecaptchaToken = async () => {
    const token = await executeRecaptcha('transfersStart');
    setToken(token);
  };
