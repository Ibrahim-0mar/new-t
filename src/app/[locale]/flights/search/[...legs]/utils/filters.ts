...
export interface FLightsFilterType {
  ...
  airports: any;
  baggages: 'all' | [number, number]
  // Hold this filter untill it been fixed by back-end
  // fareRestrictions: 'all' | string[],
}
...
export const filterByArrivalAirports = (
  ...
  if (filters.length === 0) {
    ...
    itineraries.map((itinerary) => {
      if (itinerary.legs.every((leg: any) => filters.includes(leg.destination))) {
...
export const filterByDepartureAirports = (
  ...
  if (filters.length === 0) {
    ...
    const filteredItineraries: any = [];
    itineraries.map((itinerary) => {
      if (itinerary.legs.every((leg: any) => filters.includes(leg.origin))) {
        filteredItineraries.push(itinerary);
      }
    })
    return filteredItineraries;
...
export const _getInitialAirports = (
  ...
  nearbyAirportCode:string
) => {
  const airports:any = {};
  legs.map((leg: any, index: number) => {
    if (index % 2 === 0) {
    const code = leg.split('-')[type === 'origin' ? 0 : 1];
    if (code.includes("current")) {
       airports[nearbyAirportCode]="city"
    } else {
      if (code.length ===4) {
        airports[code.substring(0,3)]="airport"
      }else{
         airports[code]="city"
      }
    }
...
// filter by baggage 
const getHanbagsItinraries = (itineraries: any, filterValue: number) => {
  return itineraries.filter((itinerary : any) => itinerary.maxBagaggeValues.totalHandbages == filterValue)
}
const getBaggagesItinraries = (itineraries: any, filterValue: number) => {
  return itineraries.filter((itinerary : any) => itinerary.maxBagaggeValues.totalBaggages == filterValue)
}

export const filterByBaggages = (
    filters: 'all' | [number, number],
    itineraries: any
  ) => {
    let baggageItineraries: any[] = [...itineraries]
    // "all" case
    if(filters === "all" || (filters[0] == 0 && filters[1] == 0)) return itineraries
    
    // number[] cases
    const handbagsFilter = filters[0]
    const baggagesFilter = filters[1]
    if(handbagsFilter > 0){
      baggageItineraries = getHanbagsItinraries(baggageItineraries, handbagsFilter)
    }
    if(baggagesFilter > 0){
      baggageItineraries = getBaggagesItinraries(baggageItineraries, baggagesFilter)
    }
   return baggageItineraries
}

// filter with Fare Restrictions
const filterItinerariesByRestrictions = (itineraries: any, filters: string[]) => {
  return itineraries.filter((itinerary: any) =>
    filters.every((restriction) => itinerary.availableRestrictions[restriction])
  );
};

export const filterByFareRestrictions = (filters: any, itineraries: any) => {
  // If no filters are provided, return all itineraries.
  if (filters === "all" || filters.length === 0 || filters.length === 3) return itineraries;
  
  // Return itineraries that match the selected restrictions.
  return filterItinerariesByRestrictions(itineraries, filters);
};


export const handleFilterData = (data: FlightResultType ,watch:any,setFilteredData:(data:any)=>void) => {
  const stopItinerates = filterByStops(
    watch('stops'),
    data?.itinerariesPrice,
  );
  const alliancesItinerates = filterByAlliances(
    watch('aliances'),
    stopItinerates,
  );
  const airlinesItinerates = filterByAirlines(
    watch('airlines'),
    alliancesItinerates,
  );
  ...
  const stopoversItinerates = filterByStopovers(
    watch('stopover'),
    bookingSitesItinerates,
  );
  const priceItinerates = filterByPrice(watch('price'), stopoversItinerates);
  const durationItinerates = filterByDuration(
    watch('duration'),
    priceItinerates,
  );
  const departureItinerates = filterByDeparture(
    watch('departure'),
    durationItinerates,
  );
  const arrivalItinerates = filterByArrival(
    watch('return'),
    departureItinerates,
  );
  const arrivalAirports = filterByArrivalAirports(
    watch('airports.arrival'),
    arrivalItinerates,
  );
  const departureAirports = filterByDepartureAirports(
    watch('airports.departure'),
    arrivalAirports,
  );
  const baggageItinerates = filterByBaggages(
    watch('baggages'),
    departureAirports,
  );

  // Hold this filter untill it been fixed by back-end 
  // const fareRestrictionsItinerates = filterByFareRestrictions(
  ...
  setFilteredData({
    ...data,
    itinerariesPrice: baggageItinerates,
  });
};