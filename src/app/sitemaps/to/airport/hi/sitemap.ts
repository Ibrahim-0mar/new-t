 ...
 * // http://localhost:3000/sitemaps/to/airport/hi/sitemap.xml

 *
 * @example
 * // In a production environment, the English sitemap is available at:
 * // http://localhost:3000/sitemaps/to/airport/hi/sitemap.xml
 * // https://travolic.com/sitemaps/to/airport/hi/sitemap.xml
 */
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  return airports.map((airport) => {
    ...
    return {
      url: `${fullUrl}/hi/cheapest-flights/to/airport/${airport.code.toLowerCase()}/${formatString(airport.name)}`,
