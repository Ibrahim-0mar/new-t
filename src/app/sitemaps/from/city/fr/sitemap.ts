 ...
 * // http://localhost:3000/sitemaps/from/city/fr/sitemap.xml/1
 ...
 * // http://localhost:3000/sitemaps/from/city/fr/sitemap/1.xml
 * // https://travolic.com/sitemaps/from/city/fr/sitemap/1.xml
 * // if the total number of cities more than 30000, then the sitemap will be split into multiple sitemaps
 * // replace "1" with the sitemap id to preview the other sitemaps
...
export default async function sitemap({
  ...
  for (let i = start; i < end; i++) {
    ...
    urls.push({
      url: `${fullUrl}/fr/cheapest-flights/from/city/${city.code}/${formatString(city.name)}`,
