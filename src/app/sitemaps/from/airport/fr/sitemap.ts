 ...
 * // http://localhost:3000/sitemaps/from/airport/fr/sitemap.xml

 *
 * @example
 * // In a production environment, the English sitemap is available at:
 * // http://localhost:3000/sitemaps/from/airport/fr/sitemap.xml
 * // https://travolic.com/sitemaps/from/airport/fr/sitemap.xml
 */
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  return airports.map((airport) => {
    ...
    return {
      url: `${fullUrl}/fr/cheapest-flights/from/airport/${airport.code.toLowerCase()}/${formatString(airport.name)}`,
