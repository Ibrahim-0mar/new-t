 ...
 * // http://localhost:3000/sitemaps/from/airport/en/sitemap.xml

 *
 * @example
 * // In a production environment, the English sitemap is available at:
 * // http://localhost:3000/sitemaps/from/airport/en/sitemap.xml
 * // https://travolic.com/sitemaps/from/airport/en/sitemap.xml
 */
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  return airports.map((airport) => {
    ...
    return {
      url: `${fullUrl}/en/cheapest-flights/from/airport/${airport.code.toLowerCase()}/${formatString(airport.name)}`,
