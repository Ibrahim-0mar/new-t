...
export const viewTicketDetailsTracking = (data: TicketViewTrackingData) => {
  const {
    adults,
    children,
    infants,
  ...
  window?.dataLayer?.push({
    event: 'flightTicketView',
    // data: tripType === "round" ? {...body, secondLeg: {departure: returnDate, arrival: formatDate(itinerary.legs[itinerary.legs.length - 1].arrival)}} : body,
